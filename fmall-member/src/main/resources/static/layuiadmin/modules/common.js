layui.define("layer", function(exports){
    ...
    var returnObj = {
         ...
         * @param {string} [region] [县容器的name名字]
         * @return {undefined} [无返回值]
         */
        showCity: function(province, city, region) {
            ...
            var htmlProvince = '',
                ...
                regionVal = $region.attr('data-area') || ''; // 县默认值
            ...
            function loadCity(city, hasDefault) {
                ...
                if (hasDefault) {
                    ...
                    defaultSelect($city, cityVal, function(code) {
                        // console.log(code)
                        // 加载默认城市数据
                        regionData = findPlace(cityData, code);
                        loadregion(regionData, true);
                    });
                }
            ...
            // 加载县数据
            function loadregion(regionData, hasDefault) {
                $region.find('option').not(':first').remove();
                htmlRegion = '';
                var regionData = regionData || [];
                regionData.forEach(function(v, i) {
                    htmlRegion += '<option value='+v.code+'>'+v.name+'</option>';
                });
                $region.append(htmlRegion);

                if (hasDefault) {
                    // 默认选中县
                    defaultSelect($region,regionVal);
                }
        ...
        getCity: function(address) {
            ...
            if (province && city && region) {
                var findRegionIndex = findIndex(AreaData[findProvinceIndex].sub[findCityIndex].sub, region);

                regionName = AreaData[findProvinceIndex].sub[findCityIndex].sub[findRegionIndex].name;
            }
